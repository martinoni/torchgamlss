"""Benchmark a hybrid neural/linear TorchGAMLSS model on CPU or CUDA."""
...
from torchgamlss import GAMLSS, MiniBatchControl, MLPPredictor, Normal
...
def _synthetic_problem(
    *,
    ...
    linear_score = inputs @ weights / math.sqrt(features)
    location = 0.35 + torch.sin(linear_score) + 0.15 * inputs[:, 0].square()
    response = location + 0.35 * torch.randn(
        rows,
        dtype=dtype,
        generator=generator,
    )
...
def run_benchmark(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    response, designs, neural_inputs, true_location = _synthetic_problem(
        rows=arguments.rows,
    ...
    with torch.no_grad():
        ...
        fitted_sigma = float(fitted["sigma"].mean())
