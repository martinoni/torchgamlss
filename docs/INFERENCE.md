...
model. For a supported Normal, Poisson, NBI, Gamma, or Beta model fitted with
`fit_laml_data()`, use:
...
log-`mu` link, NBI and Gamma models with log-`mu`/log-`sigma` links, and Beta models
with logit-`mu`/logit-`sigma` links. It is
