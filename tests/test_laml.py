...
def test_bct_implicit_outer_derivatives_match_finite_difference():
    dtype = torch.float64
    ...
    initial_coefficients = torch.tensor(
        [3.0, 0.7, 0.0, 0.0, -1.5, 0.18, 0.35, 0.2, math.log(4.0)],
        dtype=dtype,
    )
...
def test_pe_implicit_outer_derivatives_match_finite_difference():
    dtype = torch.float64
    ...
    mu_penalty[2:4, 2:4] = torch.eye(2, dtype=dtype)
    initial_coefficients = torch.tensor(
        [1.5, 0.9, 0.0, 0.0, -0.55, 0.18, math.log(1.6), 0.22],
        dtype=dtype,
    )
    ...
    torch.testing.assert_close(
        implicit.outer_hessian,
        finite_difference.outer_hessian,
        # This audit shares BCPE's regularized-gamma CDF path.
        rtol=1.2e-2,
        atol=3e-4,
    )
    assert implicit.profile_evaluations < finite_difference.profile_evaluations
...
def test_gg_implicit_outer_derivatives_match_finite_difference():
    dtype = torch.float64
    data = pd.read_csv(REFERENCE_DIR / "gg_fit_data.csv")
    response = torch.tensor(data["y"].to_numpy(), dtype=dtype)
    x = torch.tensor(data["x"].to_numpy(), dtype=dtype)
    z = torch.tensor(data["z"].to_numpy(), dtype=dtype)
    mu_design = torch.column_stack(
    ...
    sigma_design = torch.column_stack((torch.ones_like(z), z))
    nu_design = torch.ones((len(data), 1), dtype=dtype)
    coefficient_count = (
        mu_design.shape[1] + sigma_design.shape[1] + nu_design.shape[1]
    )
    mu_penalty = torch.zeros((coefficient_count, coefficient_count), dtype=dtype)
    mu_penalty[2:4, 2:4] = torch.eye(2, dtype=dtype)
    initial_coefficients = torch.tensor(
        [0.5, 0.35, 0.0, 0.0, -0.55, 0.12, 0.7],
        dtype=dtype,
    )
...
def test_logno_laml_equals_normal_laml_on_log_response():
    dtype = torch.float64
    data = pd.read_csv(REFERENCE_DIR / "logno_fit_data.csv")
    response = torch.tensor(data["y"].to_numpy(), dtype=dtype)
    weights = torch.tensor(data["weight"].to_numpy(), dtype=dtype)
    ...
    sigma_design = torch.column_stack((torch.ones_like(z), z))
    mu_penalty = torch.zeros((6, 6), dtype=dtype)
    mu_penalty[2:4, 2:4] = torch.eye(2, dtype=dtype)
    offsets = {
        "mu": torch.tensor(data["mu_offset"].to_numpy(), dtype=dtype),
        "sigma": torch.tensor(data["sigma_offset"].to_numpy(), dtype=dtype),
    }
    initial_coefficients = torch.tensor(
        [0.4, 0.7, -0.25, 0.15, -0.65, 0.15],
        dtype=dtype,
    ...
    log_jacobian = (weights * response.log()).sum()

    assert log_normal.outer_converged
    assert normal.outer_converged
    torch.testing.assert_close(log_normal.coefficients, normal.coefficients)
    torch.testing.assert_close(
        log_normal.smoothing_parameters,
        normal.smoothing_parameters,
    )
    torch.testing.assert_close(log_normal.outer_gradient, normal.outer_gradient)
    torch.testing.assert_close(log_normal.outer_hessian, normal.outer_hessian)
    torch.testing.assert_close(
        log_normal.objective,
        normal.objective + log_jacobian,
    )
    torch.testing.assert_close(
        log_normal.log_likelihood,
        normal.log_likelihood - log_jacobian,
    )
    for parameter in log_normal.parameter_names:
        torch.testing.assert_close(
...
def test_formula_bct_laml_selects_lambda_and_bootstraps():
    data = pd.read_csv(REFERENCE_DIR / "bct_fit_data.csv")
    model = GAMLSS.from_formula(
        BCT(),
        {
...
def test_bct_fixed_lambda_laml_matches_r_gamlss_reference():
    data = pd.read_csv(REFERENCE_DIR / "bct_fit_data.csv")
    fitted_reference = pd.read_csv(
    ...
    for parameter in result.parameter_names:
        relative_tolerance = 6e-5 if parameter == "tau" else 3e-6
        absolute_tolerance = 3e-4 if parameter == "tau" else 3e-6
        torch.testing.assert_close(
            result.fitted_parameters[parameter],
            torch.tensor(fitted_reference[parameter].to_numpy(), dtype=torch.float64),
...
def test_formula_pe_laml_selects_lambda_and_bootstraps():
    data = pd.read_csv(REFERENCE_DIR / "pe_fit_data.csv")
    model = GAMLSS.from_formula(
        PE(),
        {
    ...
    bootstrap = model.smooth_bootstrap_data(
        ...
        algorithm="laml",
        control=control,
        generator=torch.Generator().manual_seed(726),
    )["mu"]["x"]
...
def test_pe_fixed_lambda_laml_matches_r_gamlss_reference():
    data = pd.read_csv(REFERENCE_DIR / "pe_fit_data.csv")
    fitted_reference = pd.read_csv(
...
def test_formula_gg_laml_selects_lambda_and_bootstraps():
    data = pd.read_csv(REFERENCE_DIR / "gg_fit_data.csv")
    model = GAMLSS.from_formula(
        GG(),
        {
...
def test_gg_fixed_lambda_laml_matches_r_gamlss_reference():
    data = pd.read_csv(REFERENCE_DIR / "gg_fit_data.csv")
    fitted_reference = pd.read_csv(
...
def test_logno_fixed_lambda_laml_matches_r_gamlss_reference():
    data = pd.read_csv(REFERENCE_DIR / "logno_fit_data.csv")
    fitted_reference = pd.read_csv(
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is not available")
def test_formula_bct_laml_runs_on_cuda():
    data = pd.read_csv(REFERENCE_DIR / "bct_fit_data.csv")
    model = GAMLSS.from_formula(
        BCT(),
        {
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is not available")
def test_formula_pe_laml_runs_on_cuda():
    data = pd.read_csv(REFERENCE_DIR / "pe_fit_data.csv")
    model = GAMLSS.from_formula(
        PE(),
        {
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is not available")
def test_formula_gg_laml_runs_on_cuda():
    data = pd.read_csv(REFERENCE_DIR / "gg_fit_data.csv")
    model = GAMLSS.from_formula(
        GG(),
        {
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is not available")
def test_formula_logno_laml_runs_on_cuda():
    data = pd.read_csv(REFERENCE_DIR / "logno_fit_data.csv")
    model = GAMLSS.from_formula(
        LOGNO(),
        {
